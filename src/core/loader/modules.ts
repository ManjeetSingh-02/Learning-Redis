...
export default async function (application: Application) {
  ...
  if (!fs.existsSync(completeModulesDirPath)) {
    console.warn(`(${completeModulesDirPath}) doesn't exist`);
    return;
  }
  ...
  if (moduleDirs.length === 0) {
    console.warn(`(${completeModulesDirPath}) has no modules`);
    return;
  }
  ...
  for (const moduleDir of moduleDirs) {
    ...
    if (!fs.existsSync(moduleFilePath)) {
      console.warn(`(${moduleDir}/${MODULE_FILE_NAME}) doesn't exist`);
      continue;
    }
    ...
    if (!module.default) {
      console.warn(`(${moduleDir}/${MODULE_FILE_NAME}) has no default export`);
      continue;
    }
    ...
    console.info(`(${moduleDir}) module loaded successfully`);
  }
}
